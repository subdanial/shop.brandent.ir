<?php
/**
 * The Edali_RT initiate the theme engine
 */
...
class Edali_RT {
	...
	public function init_hooks() {
        ...
        if ( 'valid' != get_option( $this->theme_slug . '_purchase_code_status', false ) ) {
            ...
            if ( ( ! isset( $_GET['page'] ) || 'edali' != $_GET['page'] ) ) {
	...
	function admin_error() {
		$out = '<div class="notice notice-error is-dismissible edali-purchase-notice"><p>' . sprintf( wp_kses_post( __( 'The %s theme needs to be registered. %sRegister Now%s', 'edali' ) ), 'Edali', '<a href="' . admin_url( 'admin.php?page=edali') . '">' , '</a>' ) . '</p></div>';
	...
	function admin_notice() {
		$out = '<div class="notice is-dismissible edali-purchase-notice"><p>' .sprintf( wp_kses_post( __( 'Purchase key is invalid. Need a license? %sPurchase Now%s', 'edali' ) ), '<a target="_blank" href="https://1.envato.market/V5Xrk">', '</a>' ) .'</p></div>';
		if ( get_option('notice_dismissed') ) {
		    return;
        }
		echo wp_kses_post($out);
	...
	function messages() {
		...
		if ( $status != '' ) {
			...
			if($status == 'valid'){
				$title = esc_html__( 'Purchase Key Verified & Registered!', 'edali' );
			}elseif($status == 'already_registered'){
				$title = esc_html__( 'Purchase Key Already Registered', 'edali' );
			}else{
				$title = esc_html__( 'Purchase Key Invalid', 'edali' );
			}
        } else {
            $license_icon = ($status == 'valid') ? '<i class="dashicons-yes"></i>' : '<i class="dashicons-warning"></i>';
		    $title = esc_html__( 'Verify Theme Purchase Key. . .', 'edali' );
        }
	...
	function form() {
		...
		<form action="" method="post" id="verify-envato-purchase" class="et-theme-register-form">
			...
			<input id="edali_purchase_code" name="edali_purchase_code" type="text" value="<?php echo esc_attr( $license ); ?>" placeholder="<?php esc_attr_e( 'Enter purchase key', 'edali' ); ?>" required>
			<?php if( $status != 'valid' ){ ?>
				<input type="submit" value="<?php esc_attr_e( 'Verify Now', 'edali' ); ?>">
			<?php } ?>
			<?php
			if ( $status != '' ) {
				if( $status == 'valid' ){ ?>
					<input id="edali_purchase_code" name="edali_purchase_code" type="hidden" value="">
					<input type='submit' class='deactivate' value='Deregister Theme'>
					<?php
				}
			} ?>

			<?php if( get_option( 'edali_purchase_code_status' ) == 'already_registered' ): ?>
				<div class="et_warning">
					<span class="dashicons dashicons-warning"></span>
					<?php echo stripslashes( get_option( 'edali_already_registered' ) ); ?>
				</div>
			<?php endif; ?>

		</form>
		<?php
        if ( isset($_POST['edali_purchase_code']) ) {
			if( $_POST['edali_purchase_code'] != '' ){
				echo "<meta http-equiv='refresh' content='0'>";
				update_option( $this->theme_slug . '_purchase_code', $_POST['edali_purchase_code'] );
				$purchase_code = htmlspecialchars($_POST['edali_purchase_code']);
				...
				if ( is_object($o) && strpos($o->item_name, 'Edali') !== false ) {
					...
					update_option( 'edali_purchase_code_status', 'invalid', 'yes' );
				}
			}else{
				echo "<meta http-equiv='refresh' content='0'>";
				
				$purchase_code = get_option( 'edali_purchase_valid_code' );
