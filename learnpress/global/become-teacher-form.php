 ...
 *
 * This template can be overridden by copying it to edali/learnpress/global/become-teacher-form.php.
 *
 * @author  ThimPress
...
<div id="learn-press-become-teacher-form" class="become-teacher-form learn-press-form <?php if( $user && ! $user instanceof LP_User_Guest && !learn_press_become_teacher_sent() ) echo 'allow'; else echo 'block-fields';?>">
    ...
    <h4 class="teacher-title"><?php esc_html_e( 'Request to become a teacher', 'edali' ); ?></h4>
    <?php if ( ! $user || $user instanceof LP_User_Guest ) { ?>
        <p class="message message-info"><i class="fa"></i><?php printf( __( 'Please <a class="login" href="%s">login</a> to send your request!', 'edali' ), add_query_arg( 'redirect_to', get_permalink(), home_url( '/login/' )) ); ?></p>
    <?php } ?>
