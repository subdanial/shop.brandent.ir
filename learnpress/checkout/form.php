 ...
 *
 * This template can be overridden by copying it to edali/learnpress/checkout/form.php.
 *
 * @author   ThimPress
		...
		<?php if ( ! is_user_logged_in() ) { ?>
            <p class="button-cancel-guest-checkout">
                <button type="button" class="lp-button lp-button"
                        id="learn-press-button-cancel-guest-checkout"><?php _e( 'Back', 'edali' ); ?></label></button>
            </p>
		<?php } ?>
        -->
    </form>
...
<?php if ( ! is_user_logged_in() && ! LP()->checkout()->is_enable_login() && ! LP()->checkout()->is_enable_register() ) { ?>
    <p><?php printf( __( 'Please login to continue checkout. %s', 'edali' ), sprintf( '<a href="%s">%s</a>', learn_press_get_login_url(), __( 'Login?', 'edali' ) ) ); ?></p>
<?php } ?>
...
<div class="guest-and-class">
    <?php if ( ! is_user_logged_in() && LP()->checkout()->is_enable_guest_checkout() ) { ?>
        <p class="button-continue-guest-checkout">
            <button type="button" class="lp-button lp-button-guest-checkout" id="learn-press-button-guest-checkout"><?php _e( 'Continue checkout as Guest?', 'edali' ); ?></label></button>
        </p>
    <?php } ?>
