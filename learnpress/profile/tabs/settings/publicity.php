 ...
 *
 * This template can be overridden by copying it to edali/learnpress/settings/tabs/change-password.php.
 *
 * @author   ThimPress
...
<form method="post" name="profile-publicity" enctype="multipart/form-data" class="learn-press-form">
    ...
    <ul class="form-fields">
        ...
        <li class="form-field">
            <label for="my-dashboard"><?php _e( 'My dashboard', 'edali' ); ?></label>
            <div class="form-field-input">
                ...
                <p class="description">
                    <?php _e( 'Public user profile content, if this option is turn off then other sections in profile also become invisible.', 'edali' ); ?>
                </p>
            </div>
        </li>
		...
		<?php if ( LP()->settings()->get( 'profile_publicity.courses' ) === 'yes' ) { ?>
            <li class="form-field">
                <label for="my-courses"><?php _e( 'My courses', 'edali' ); ?></label>
                <div class="form-field-input">
                    <input type="checkbox" name="publicity[courses]" value="yes"
                           id="my-courses" <?php checked( $profile->get_publicity( 'courses' ), 'yes' ); ?>/>
                    <p class="description"><?php _e( 'Public your profile courses', 'edali' ); ?></p>
                </div>
            </li>
		<?php } ?>
		...
		<?php if ( LP()->settings()->get( 'profile_publicity.quizzes' ) === 'yes' ) { ?>
            <li class="form-field">
                <label for="my-quizzes"><?php _e( 'My quizzes', 'edali' ); ?></label>
                <div class="form-field-input">
                    <input name="publicity[quizzes]" value="yes" type="checkbox"
                           id="my-quizzes" <?php checked( $profile->get_publicity( 'quizzes' ), 'yes' ); ?>/>
                    <p class="description"><?php _e( 'Public your profile quizzes', 'edali' ); ?></p>
                </div>
            </li>
		<?php } ?>
    ...
    <button type="submit" name="submit" id="submit"><?php _e( 'Save changes', 'edali' ); ?></button>
